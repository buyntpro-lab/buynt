...
```sql
-- Drop old functions first (they exist with different return types)
DROP FUNCTION IF EXISTS public.mark_handoff_uploaded(uuid, text);
DROP FUNCTION IF EXISTS public.mark_return_uploaded(uuid, text);
DROP FUNCTION IF EXISTS public.confirm_handoff(uuid);
DROP FUNCTION IF EXISTS public.confirm_return(uuid);
DROP FUNCTION IF EXISTS public.complete_rental(uuid);

-- Now create the new versions with JSONB return type
